...
> Source: `plugins/vertical-plugins/regulatory-legal/skills/reg-feed-watcher/SKILL.md`.
...
1. Load `the installed practice profile for regulatory-legal` → watchlist, materiality threshold, feed config.
...
`the installed practice profile for regulatory-legal` → watchlist, materiality threshold, feed configuration, digest output path (if set).
...
> **Coverage gap noticed:** Your watchlist includes [category], but only [N] feeds are configured. The source catalog lists [X] options in this category (e.g., [top 2-3 names]). Want me to suggest additions? Run `/regulatory-legal:cold-start-interview --redo` to update, or edit `the installed practice profile for regulatory-legal` directly.
...
Each item gets a materiality tier per `the installed practice profile for regulatory-legal`:
...
1. Look for `Digest output path` in `the installed practice profile for regulatory-legal`. If set, write there. Default if unset: `~/regulatory-legal-digests/reg-digest-YYYY-MM-DD.md`.
...
This skill reads the watchlist, materiality threshold, and feed configuration from `the installed practice profile for regulatory-legal`. When a required value is still `[PLACEHOLDER]` or empty, say so in the output — specifically, not generically:

- **Watchlist empty:** stop and say "The watchlist in your configuration is empty. I can't pull feeds without knowing which regulators to watch. Run `/regulatory-legal:cold-start-interview --redo` or edit `the installed practice profile for regulatory-legal` and add at least one regulator."
- **Materiality threshold empty:** fall back to the default tiers and append: "This output used the default materiality tiers because your configuration doesn't have custom thresholds set. Tune them with `/regulatory-legal:cold-start-interview --redo` or by editing `the installed practice profile for regulatory-legal`."
- **Feed configuration empty:** run Federal Register API only and append: "This output used only the free Federal Register API because your configuration doesn't list direct RSS or paid feeds. Add feeds with `/regulatory-legal:cold-start-interview --redo` or by editing `the installed practice profile for regulatory-legal`."
