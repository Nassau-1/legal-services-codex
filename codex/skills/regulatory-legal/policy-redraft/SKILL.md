...
> Source: `plugins/vertical-plugins/regulatory-legal/skills/policy-redraft/SKILL.md`.
...
1. Load `the installed practice profile for regulatory-legal` → policy library index + practice profile.
...
This skill reads the policy library index and owners from `the installed practice profile for regulatory-legal`. When a value it needs is empty or still `[PLACEHOLDER]`:
