...
> Source: `plugins/vertical-plugins/law-student/skills/cold-start-interview/SKILL.md`.
...
5. Write `the installed practice profile for law-student` (creating parent directories as needed), including `## Who's using this` and `## Available integrations`. Add `LIMITED DATA` flag if fewer than 10 materials were shared.
6. Confirm with the user: "Here's what I captured — anything wrong?"

**`--check-integrations`:** Re-run only the Part 0 integration-availability check. Updates `## Available integrations` in `the installed practice profile for law-student` without touching the role or the rest of the profile. Use after adding or removing an MCP connector.
...
Read `the installed practice profile for law-student`:
...
- **Pause and resume.** Tell the student up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/law-student:cold-start-interview` again later and I'll pick up where you left off." When the student pauses, write a partial configuration to `the installed practice profile for law-student` with a `<!-- SETUP PAUSED AT: [section name] — run /law-student:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the student: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> Done. Your configuration is at `the installed practice profile for law-student` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
