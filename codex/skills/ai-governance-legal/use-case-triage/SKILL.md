...
> Source: `plugins/vertical-plugins/ai-governance-legal/skills/use-case-triage/SKILL.md`.
...
1. Read `the installed practice profile for ai-governance-legal`. Confirm registry is populated — if not, stop and direct to setup.
...
## Read `the installed practice profile for ai-governance-legal` first

Before triaging, always read `the installed practice profile for ai-governance-legal`. The use case registry and red lines there
...
Check the use case registry in `the installed practice profile for ai-governance-legal` for a direct or close match.
...
The APPROVED / CONDITIONAL / NOT APPROVED buckets, the red-line definitions, and the CONDITIONAL required-controls list all come from `the installed practice profile for ai-governance-legal` → `## AI use case triage criteria` and `## Use case registry`. If the playbook doesn't define a criterion the use case turns on, ask the user: "Your playbook doesn't cover [specific question]. What's your default position? I'll add it to `the installed practice profile for ai-governance-legal` so the next triage is consistent."

**Before issuing an APPROVED classification (approving an AI use case for deployment):** Read `## Who's using this` in `the installed practice profile for ai-governance-legal`. If the Role is Non-lawyer:
...
**Governance tier:** [Standard / Elevated / High — per `the installed practice profile for ai-governance-legal`]
...
> "Add to `the installed practice profile for ai-governance-legal` → Use case registry. This means next time the same request
