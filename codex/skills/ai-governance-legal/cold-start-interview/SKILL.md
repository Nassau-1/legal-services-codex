...
> Source: `plugins/vertical-plugins/ai-governance-legal/skills/cold-start-interview/SKILL.md`.
...
description: >
  ...
  `the installed practice profile for ai-governance-legal` from
...
1. Check `the installed practice profile for ai-governance-legal` — if populated and no `--redo`, confirm before overwriting.
...
6. Write `the installed practice profile for ai-governance-legal` (create parent directories as needed). Show summary. Offer first task.

## Flags

- `--redo` — re-run the full interview and overwrite `the installed practice profile for ai-governance-legal`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `the installed practice profile for ai-governance-legal` without re-running the full interview. Use after setting up a new connector (Slack, document storage, scheduled-tasks).
...
Read `the installed practice profile for ai-governance-legal`:
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/ai-governance-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `the installed practice profile for ai-governance-legal` with a `<!-- SETUP PAUSED AT: [section name] — run /ai-governance-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
5. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your configuration is at `the installed practice profile for ai-governance-legal` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
