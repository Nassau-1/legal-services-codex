...
> Source: `plugins/vertical-plugins/litigation-legal/skills/subpoena-triage/SKILL.md`.
...
3. If grand jury → stop, escalate per `the installed practice profile for litigation-legal`. Otherwise continue.
4. Load `~/.claude/plugins/config/claude-for-legal/litigation-legal/matters/_log.yaml` for cross-check. Load `the installed practice profile for litigation-legal` → landscape, privilege conventions, escalation norms.
...
- `the installed practice profile for litigation-legal` → landscape (regulators we deal with), house privilege conventions, escalation norms
...
- Privilege log will be required — flag the format per `the installed practice profile for litigation-legal`
...
**Privilege log format required:** [per `the installed practice profile for litigation-legal`]
...
**Before responding to the subpoena (serving objections, producing documents, appearing for deposition, or filing a motion to quash — any substantive response to the issuing party or court):** Read `## Who's using this` in `the installed practice profile for litigation-legal`. If the Role is Non-lawyer:
...
- If classified as **grand jury subpoena** → stop, flag for escalation per `the installed practice profile for litigation-legal`, do not proceed with standard triage.
