...
> Source: `plugins/vertical-plugins/litigation-legal/skills/privilege-log-review/SKILL.md`.
...
1. Load `the installed practice profile for litigation-legal` → review protocol, priv log format.
...
`the installed practice profile for litigation-legal` → privilege log format, review protocol.
...
**Before the privilege log is served on the opposing party (the consequential act — this includes serving the log AND designating documents withheld or produced under a protective-order designation such as Confidential / Highly Confidential / AEO):** Read `## Who's using this` in `the installed practice profile for litigation-legal`. If the Role is Non-lawyer:
