...
> Source: `plugins/vertical-plugins/litigation-legal/skills/legal-hold/SKILL.md`.
...
3. Load `the installed practice profile for litigation-legal` → privilege markings, hold template pointer, escalation norms.
...
- `the installed practice profile for litigation-legal` — house style for litigation hold template pointer, privilege marking, escalation norms
...
**Before issuing the hold to custodians (the consequential act):** Read `## Who's using this` in `the installed practice profile for litigation-legal`. If the Role is Non-lawyer:
...
**Draft the notice** to each custodian, using the house template in `the installed practice profile for litigation-legal` if one is configured; otherwise the default template below.
...
DATE: [effective date]
TO: [custodian name]
FROM: [signer — per `the installed practice profile for litigation-legal` default]
...
**Before releasing the hold (the consequential act — preservation obligations resume normal retention):** Read `## Who's using this` in `the installed practice profile for litigation-legal`. If the Role is Non-lawyer:
