...
> Source: `plugins/vertical-plugins/legal-clinic/skills/cold-start-interview/SKILL.md`.
...
1. Check `the installed practice profile for legal-clinic`. If populated and no `--redo`, confirm before overwriting.
...
6. Write `the installed practice profile for legal-clinic` including `## Who's using this` and `## Available integrations`. Show supervision choice and practice-area templates for confirmation.
...
Read `the installed practice profile for legal-clinic`:
...
- **Pause and resume.** Tell the supervising attorney up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/legal-clinic:cold-start-interview` again later and I'll pick up where you left off." When the attorney pauses, write a partial configuration to `the installed practice profile for legal-clinic` with a `<!-- SETUP PAUSED AT: [section name] — run /legal-clinic:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the attorney: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
> Done. Your clinic's configuration is at `the installed practice profile for legal-clinic` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
