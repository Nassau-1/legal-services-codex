...
> Source: `plugins/vertical-plugins/legal-clinic/skills/supervisor-review-queue/SKILL.md`.
...
1. Check `the installed practice profile for legal-clinic` → supervision style. If NOT "formal review queue": explain the clinic is set up for [flags/lighter-touch], no formal queue exists, and how to switch.
...
**This skill is only active if `the installed practice profile for legal-clinic` → Supervision style is "formal review queue."** Otherwise it's dormant — the cold-start interview asks the professor which model they want, and this is one of three options.
