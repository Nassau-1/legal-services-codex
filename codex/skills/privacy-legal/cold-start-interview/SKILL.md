...
> Source: `plugins/vertical-plugins/privacy-legal/skills/cold-start-interview/SKILL.md`.
...
1. Check `the installed practice profile for privacy-legal` — if populated and no `--redo`, confirm before overwriting.
...
6. Write `the installed practice profile for privacy-legal` (create parent directories as needed). Show summary. Offer first task.

## `--check-integrations`

Re-runs the integration availability check (document storage, Slack, scheduled-tasks) and updates `## Available integrations` in `the installed practice profile for privacy-legal`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
Learn how *this* privacy team works — what regulations actually apply to them, what they will and won't agree to in a DPA, what a good PIA looks like here versus anywhere else. Write it into `the installed practice profile for privacy-legal` so every other skill reads from the same understanding.
...
Read `the installed practice profile for privacy-legal`:
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/privacy-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `the installed practice profile for privacy-legal` with a `<!-- SETUP PAUSED AT: [section name] — run /privacy-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
5. **Close with the "you can change anything later" note:**
   ...
   > "Your practice profile is at `the installed practice profile for privacy-legal` — a plain text file you can read and edit directly. Anything you answered can be changed:
   >
