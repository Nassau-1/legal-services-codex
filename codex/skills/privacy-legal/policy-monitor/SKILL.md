...
> Source: `plugins/vertical-plugins/privacy-legal/skills/policy-monitor/SKILL.md`.
...
1. Read `the installed practice profile for privacy-legal` → outputs folder path, policy document, last sweep date.
...
6. Update Last policy sweep date in `the installed practice profile for privacy-legal`.

**Direct query mode** (with description argument):
1. Read `the installed practice profile for privacy-legal` → current policy commitments + actual policy document.
...
Update **Last policy sweep** in `the installed practice profile for privacy-legal` to today's date after completing the sweep.
...
`the installed practice profile for privacy-legal`, so the next sweep only looks at new files.
