...
> Source: `plugins/vertical-plugins/privacy-legal/skills/reg-gap-analysis/SKILL.md`.
...
1. Load `the installed practice profile for privacy-legal` → privacy policy commitments, regulatory footprint, DSAR systems.
...
This skill diffs the new requirement against what you currently do (per `the installed practice profile for privacy-legal` → Privacy policy commitments + the practices documented in PIAs) and produces a gap list with a remediation plan.
