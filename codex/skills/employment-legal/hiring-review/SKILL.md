...
> Source: `plugins/vertical-plugins/employment-legal/skills/hiring-review/SKILL.md`.
...
1. Load `the installed practice profile for employment-legal` → jurisdictional footprint, hiring review triggers, restrictive covenant policy.
...
`the installed practice profile for employment-legal` → jurisdictional footprint, hiring review triggers, restrictive
covenant policy, offer letter template location.

## Output header

Prepend the work-product header from `the installed practice profile for employment-legal` → `## Outputs` (it differs by user role — see `## Who's using this`).
...
Check the jurisdiction table in `the installed practice profile for employment-legal` for this state/country. If it's
...
Per `the installed practice profile for employment-legal` restrictive covenant policy: does this hire even get one?
...
Check the `the installed practice profile for employment-legal` table for this jurisdiction. Common categories to
...
**Before producing a "Clear to send" recommendation or a final offer letter for signature:** Read `## Who's using this` in `the installed practice profile for employment-legal`. If the Role is **Non-lawyer**:
