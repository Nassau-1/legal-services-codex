...
> Source: `plugins/vertical-plugins/legal-builder-hub/skills/auto-updater/SKILL.md`.
...
1. Load `the installed practice profile for legal-builder-hub` → installed skills + auto-update prefs.
...
`the installed practice profile for legal-builder-hub` → installed skills (with version/commit SHA), update preferences (notify / manual).
...
`the installed practice profile for legal-builder-hub` →
`## Freshness reminders`.
...
Replace the installed skill files with the new version. Update `the installed practice profile for legal-builder-hub` installed list with the new commit SHA. Backup the old version first (to `~/.claude/skills/.backups/[skill]-[old-sha]/`) in case of rollback.
