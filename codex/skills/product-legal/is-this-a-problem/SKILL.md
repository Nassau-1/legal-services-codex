...
> Source: `plugins/vertical-plugins/product-legal/skills/is-this-a-problem/SKILL.md`.
...
1. Load `the installed practice profile for product-legal` → Risk calibration.
...
Read `the installed practice profile for product-legal` → `## Risk calibration`. The whole point of this skill is pattern-matching against that table.
...
Slack triage replies are internal legal advice. If the reply is being pasted into a ticket, document, or channel that's broadly shared with non-legal, prepend the work-product header from `the installed practice profile for product-legal` `## Outputs` (it differs by user role — see `## Who's using this`):
