...
> Source: `plugins/vertical-plugins/corporate-legal/skills/board-minutes/SKILL.md`.
...
- `the installed practice profile for corporate-legal` → `## Board & Secretary` section:
...
- If `the installed practice profile for corporate-legal` has no minutes format: run cold-start first. Do not proceed with a generic format.
...
**Directors present:**
- Pull from board composition in `the installed practice profile for corporate-legal` as the starting point
...
Use the house format from `the installed practice profile for corporate-legal`. Do not default to a generic format. The seed minutes are the template — replicate the structure, the header, the resolution language, the level of discussion detail.
...
**Previous minutes:**
Standard language: approval of minutes from prior meeting. Pull date of prior meeting from `the installed practice profile for corporate-legal` board calendar if available, otherwise leave as [DATE OF PRIOR MEETING].
...
RESOLVED, that [resolution text in house language from `the installed practice profile for corporate-legal`].
```
...
**Before adopting minutes as final:** Read `## Who's using this` in `the installed practice profile for corporate-legal`. If the Role is **Non-lawyer**:
...
Produce the full draft. The minutes themselves are a corporate record, not privileged; do not apply the work-product header to the minutes as circulated. The drafting notes, placeholder flags, and review checklist below are work product — prepend the work-product header from `the installed practice profile for corporate-legal` `## Outputs` (it differs by user role — see `## Who's using this`):
