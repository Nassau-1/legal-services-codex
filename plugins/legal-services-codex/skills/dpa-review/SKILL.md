...
> Source: `plugins/vertical-plugins/privacy-legal/skills/dpa-review/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /dpa-review

1. Load `the installed practice profile for privacy-legal` → DPA playbook. If placeholders, stop and prompt setup.
...
DPAs come in two flavors and the review is nearly opposite for each. When a customer sends their DPA, we're defending our operational flexibility. When we send one to a vendor, we're protecting our (and our customers') data. Both reviews read from the same `the installed practice profile for privacy-legal` playbook but from opposite rows.

## First: which direction?

Before anything else, establish:

- **We are the processor** → customer is sending us their DPA → read `the installed practice profile for privacy-legal` → "When we are the processor" table
...
Before reviewing, check the outputs folder for prior work on this counterparty or processing activity. Read `the installed practice profile for privacy-legal` → `## Outputs` for the outputs folder path. Scan for:
...
**Carry severity from the upstream output as a floor** per the cross-skill severity floor rule in `the installed practice profile for privacy-legal` → `## Shared guardrails`. A processing activity the triage rated 🔴 cannot be quietly downgraded to 🟢 in the DPA review; any demotion is stated and explained.
...
Read `the installed practice profile for privacy-legal` → `## DPA playbook`. Also read `## Privacy policy commitments` — the DPA can't contradict what the privacy policy promises.
...
Prepend the work-product header from `the installed practice profile for privacy-legal` `## Outputs` (it differs by user role — see `## Who's using this`).
...
**Before proceeding to sign or countersign a DPA (including returning an executed version, consenting to automatic execution on a counterparty platform, or instructing a signatory to execute):** Read `## Who's using this` in `the installed practice profile for privacy-legal`. If the Role is Non-lawyer:
