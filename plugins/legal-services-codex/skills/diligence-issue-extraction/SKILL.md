...
> Source: `plugins/vertical-plugins/corporate-legal/skills/diligence-issue-extraction/SKILL.md`.
...
1. Load `the installed practice profile for corporate-legal` + `~/.claude/plugins/config/claude-for-legal/corporate-legal/deals/[code]/deal-context.md`.
...
The VDR has 2,000 documents. Somewhere in there are the 30 that matter for the deal. This skill reads documents against the diligence categories and materiality thresholds from `the installed practice profile for corporate-legal`, extracts issues, and writes them in house memo format.

## Load context

- `the installed practice profile for corporate-legal` → Diligence structure (categories, materiality thresholds)
- `the installed practice profile for corporate-legal` → Issues memo format (how findings are stated)
...
Per `the installed practice profile for corporate-legal` / deal-context thresholds. Don't review everything if the threshold says contracts >$X.
...
Per the finding template in `the installed practice profile for corporate-legal`. If the seed memo used this:
...
- **To ai-tool-handoff:** If Luminance/Kira is in use per `the installed practice profile for corporate-legal`, hand bulk contract review there. This skill handles the nuanced documents (side letters, amendments, anything the AI tool struggles with).
...
- It doesn't replace bulk AI review. For high-volume clause extraction, hand off to Luminance/Kira per `the installed practice profile for corporate-legal`. This skill is for the judgment layer.
