...
> Source: `plugins/vertical-plugins/ai-governance-legal/skills/vendor-ai-review/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /vendor-ai-review

1. Read `the installed practice profile for ai-governance-legal`. Confirm vendor governance positions are populated — if not, stop and direct to setup.
...
6. AI policy consistency diff vs. `the installed practice profile for ai-governance-legal`.
...
Read `the installed practice profile for ai-governance-legal` → `## Vendor AI governance`. Also read `## AI policy commitments`
— vendor terms can't be consistent with a use restriction our own policy imposes if
we've agreed to something different.

If `the installed practice profile for ai-governance-legal` contains `[PLACEHOLDER]`, surface this bounce:
...
Review each term below. For each, extract what the vendor's contract actually says and compare it against the position in `the installed practice profile for ai-governance-legal` → `## Vendor AI governance` (standard / acceptable fallback / automatic no). The default positions come from the team's playbook, not from this skill.
...
If `the installed practice profile for ai-governance-legal` doesn't define a position for a term on this list, ask: "Your playbook doesn't cover [term]. What's your default position, your acceptable fallback, and your automatic no? I'll add it to `the installed practice profile for ai-governance-legal` so the next review is consistent."

---

## Playbook comparison

For each term above, compare what we found to the positions in `the installed practice profile for ai-governance-legal`.
...
> **Our position:** [from `the installed practice profile for ai-governance-legal`]
> **Gap:** [specific delta — or "Aligned"]
> **Proposed fix:** [specific redline language, or "escalate — outside fallback"]

Use the severity ratings consistently (calibrated against `the installed practice profile for ai-governance-legal` positions):
...
- 🔴 **Critical** — outside fallback; deployment should not proceed without resolution. Escalate per `the installed practice profile for ai-governance-legal`.
...
Cross-check the vendor's terms against our AI policy commitments in `the installed practice profile for ai-governance-legal`.
...
**Before recommending signature of a vendor AI agreement (the version the company will execute):** Read `## Who's using this` in `the installed practice profile for ai-governance-legal`. If the Role is Non-lawyer:
...
[For each 🔴 and 🟠: the fallback from `the installed practice profile for ai-governance-legal`, or "escalate — outside fallback"
...
- It doesn't decide whether to accept terms outside the fallbacks. It routes those
  per the escalation table in `the installed practice profile for ai-governance-legal`.
