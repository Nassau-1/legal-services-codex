...
> Source: `plugins/vertical-plugins/privacy-legal/skills/pia-generation/SKILL.md`.
...
1. Load `the installed practice profile for privacy-legal` → PIA house style (trigger, structure, depth, sign-off).
...
Before writing a new PIA, check the outputs folder for prior work on the same feature, processing activity, or counterparty. Read `the installed practice profile for privacy-legal` → `## Outputs` for the path. Scan for:
...
**Carry severity from upstream as a floor** per the cross-skill severity floor rule in `the installed practice profile for privacy-legal` → `## Shared guardrails`. A use-case-triage that rated the activity high-risk cannot become a PIA that concludes low-risk without stating why and what changed.
...
Read `the installed practice profile for privacy-legal` → `## PIA house style`. That has:
...
Check the trigger criteria in `the installed practice profile for privacy-legal`. That is the team's house answer.
...
**Use the seed PIA structure from the config CLAUDE.md.** If none was captured, use this default. Prepend the work-product header from `the installed practice profile for privacy-legal` `## Outputs` (it differs by user role — see `## Who's using this`).
...
Every PIA should cross-check against the privacy policy commitments in `the installed practice profile for privacy-legal`. The common drift:
...
- **To the sign-off process:** Per `the installed practice profile for privacy-legal` → who approves PIAs.
...
**Before proceeding to submit a DPIA (or any equivalent impact assessment) to a regulator, supervisory authority, or enforcement body:** Read `## Who's using this` in `the installed practice profile for privacy-legal`. If the Role is Non-lawyer:
