...
> Source: `plugins/vertical-plugins/commercial-legal/skills/amendment-history/SKILL.md`.
...
This skill reads the base agreement and amendments — often privileged or confidential in their own right, and typically used for privileged analysis. The output inherits the source's privilege and confidentiality status. Prepend the work-product header from `the installed practice profile for commercial-legal` `## Outputs` to every output below, distribute only within the privilege circle, and store it where privileged materials live. Strip the header before any external delivery.
...
- It does not compare against the playbook in `the installed practice profile for commercial-legal` — that is the
