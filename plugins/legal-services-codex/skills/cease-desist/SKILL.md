...
> Source: `plugins/vertical-plugins/ip-legal/skills/cease-desist/SKILL.md`.
...
1. **Read the practice profile.** Load `the installed practice profile for ip-legal`. If it contains `[PLACEHOLDER]` markers or does not exist, stop and say: "This plugin needs setup before it can give you useful output. Run `/ip-legal:cold-start-interview` — the C&D skill depends on your enforcement posture, approval matrix, and practice-area mix, none of which are configured yet."
