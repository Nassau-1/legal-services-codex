...
> Source: `plugins/vertical-plugins/litigation-legal/skills/brief-section-drafter/SKILL.md`.
...
1. Load `the installed practice profile for litigation-legal` → case theory, house style.
...
`the installed practice profile for litigation-legal` → case theory, house style (citation format, structure, tone, length norms).
...
Per `the installed practice profile for litigation-legal`:
...
**Before the brief is filed (the consequential act — this skill drafts, but the gate runs at the filing step regardless of who triggers it):** Read `## Who's using this` in `the installed practice profile for litigation-legal`. If the Role is Non-lawyer:
