...
> Source: `plugins/vertical-plugins/litigation-legal/skills/demand-intake/SKILL.md`.
...
1. Load `the installed practice profile for litigation-legal` → demand-letter practice, landscape, risk calibration.
...
- Desired outcome dollar value ≥ the medium-severity band from `the installed practice profile for litigation-legal` risk calibration
- Counterparty is a customer, competitor, or frequent adversary per `the installed practice profile for litigation-legal` landscape
- User ran with `--full`
...
- Run the conflicts check. If the counterparty is a customer or known entity, flag that this should clear conflicts (per `the installed practice profile for litigation-legal`) before sending — but the check itself lives in the matter-intake workflow or outside this skill.
