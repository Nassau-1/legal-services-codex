...
> Source: `plugins/vertical-plugins/privacy-legal/skills/use-case-triage/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /use-case-triage

1. Read `the installed practice profile for privacy-legal`. Confirm privacy practice is configured — if not, stop and direct to setup.
...
Before triaging, always read `the installed practice profile for privacy-legal`. The PIA trigger criteria, regulatory
...
Read `the installed practice profile for privacy-legal` → `## PIA house style` → Trigger criteria. Apply them.
...
Read `the installed practice profile for privacy-legal` → `## Privacy policy commitments`. Check the proposed activity
