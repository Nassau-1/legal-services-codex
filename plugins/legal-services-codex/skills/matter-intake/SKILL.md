...
> Source: `plugins/vertical-plugins/litigation-legal/skills/matter-intake/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /matter-intake

1. Load `the installed practice profile for litigation-legal` → risk calibration (for triage), landscape (for context, conflicts method), stakeholders (for who to loop in).
...
- `the installed practice profile for litigation-legal` — risk calibration (triage thresholds, materiality, settlement ladder), landscape (stakeholders, outside counsel bench).
...
Before going further, run the conflicts step per `the installed practice profile for litigation-legal` → Conflicts clearance.

- **Status:** `cleared | pending | not-run | waived`
- **Method:** match what `the installed practice profile for litigation-legal` declares (`corporate-legal | outside-counsel | system-check | informal | other`). If the declared method is `informal`, say so — the record still captures that a counsel's-judgment check was the basis.
...
- `not-run` → **STOP. This is a gate.** The skill will not create `matter.md`, `history.md`, or a `_log.yaml` entry until the conflicts posture is resolved. Three acceptable paths:
  ...
  **Path 1 — Run conflicts now.** Pause this intake. Clear per `the installed practice profile for litigation-legal` Conflicts clearance. Return with `status: cleared` or `status: waived` with rationale.

  **Path 2 — Mark pending with owner + due date.** Allowed only when `the installed practice profile for litigation-legal` Conflicts clearance declares parallel-intake acceptable. Capture: who is running conflicts, when they're expected to return, what entities they're checking. Intake proceeds; matter row carries `conflicts.status: pending`; `/portfolio-status` flags it every run; `/matter-update` re-prompts until resolved.
...
- Severity: high | medium | low (reference the `the installed practice profile for litigation-legal` severity bands)
- Likelihood: high | medium | low (reference the `the installed practice profile for litigation-legal` likelihood bands)
...
If the risk calibration in `the installed practice profile for litigation-legal` is thin, don't fake precision. Use the user's gut and note the thinness.

### 5. Materiality

Against the house thresholds in `the installed practice profile for litigation-legal`:
...
From `the installed practice profile for litigation-legal` landscape — which internal stakeholders are involved?
- Business lead
