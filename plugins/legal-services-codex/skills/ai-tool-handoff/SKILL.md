...
description: Detects when Luminance, Kira, or a similar bulk-review tool is in use, hands off the high-volume clause extraction to it, and QAs its output per the trust level in `the installed practice profile for corporate-legal`. Use when user says "send to Luminance", "bulk review", "AI extraction", or when diligence-issue-extraction hits a high-volume category.
---

> Generated by `scripts/build_codex_port.py` on `2026-05-14T16:29:15.363024+02:00`.
> Source: `plugins/vertical-plugins/corporate-legal/skills/ai-tool-handoff/SKILL.md`.
...
`the installed practice profile for corporate-legal` → AI-assisted review:
...
If `the installed practice profile for corporate-legal` says no AI tool → this skill is a no-op. Everything goes through diligence-issue-extraction directly.
...
- Specify extraction targets per `the installed practice profile for corporate-legal` (which clause types)
- Note the materiality threshold so tool output can be filtered

### Step 2: Load (or instruct the loader)

Per `the installed practice profile for corporate-legal` — who loads. If it's you, generate the load instructions. If it's someone else, generate the request:
...
- Exclusivity
- [etc. per `the installed practice profile for corporate-legal`]
...
**"Use as-is":** Ingest directly into diligence findings. (Only if `the installed practice profile for corporate-legal` says this — it's rare.)
...
**Trust level:** [per `the installed practice profile for corporate-legal`]
...
- It doesn't replace the tool's output with its own judgment entirely — if `the installed practice profile for corporate-legal` says spot-check 10%, check 10%, not 100%.
- It doesn't decide the trust level — that's in `the installed practice profile for corporate-legal`, set at cold-start based on the team's experience with the tool.
