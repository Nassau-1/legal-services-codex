...
description: Route a contract issue to the right approver per the escalation matrix in `the installed practice profile for commercial-legal`, and draft the ask. Use when the user says "who needs to approve this", "escalate this", "does this need GC sign-off", "route this for approval", or when another skill finds an issue that exceeds the reviewer's authority.
---
