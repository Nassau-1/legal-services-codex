...
> Source: `plugins/vertical-plugins/litigation-legal/skills/matter-update/SKILL.md`.
...
- `the installed practice profile for litigation-legal` — risk calibration (if re-assessing risk)
...
If the Strategy update is a **settlement acceptance** (the company is accepting a settlement offer, executing a settlement agreement, or authorizing acceptance in principle — not merely logging an offer made or received): Read `## Who's using this` in `the installed practice profile for litigation-legal`. If the Role is Non-lawyer:
...
If materiality moves to `reserved` or `disclosed`, and the matter did not previously carry a reserve or disclosure, flag the event as requiring finance / audit-committee notification per `the installed practice profile for litigation-legal` materiality thresholds.
