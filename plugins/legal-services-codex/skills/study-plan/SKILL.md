...
> Source: `plugins/vertical-plugins/law-student/skills/study-plan/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /study-plan

1. Load `the installed practice profile for law-student` → bar jurisdiction, exam format, bar date, weak subjects, target study hours/day, prep course.
...
`the installed practice profile for law-student`:
