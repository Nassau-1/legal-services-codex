...
> Source: `plugins/vertical-plugins/litigation-legal/skills/deposition-prep/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /deposition-prep

1. Load `the installed practice profile for litigation-legal` → case theory, key facts.
...
`the installed practice profile for litigation-legal` → case theory (theory, pivot fact, key facts for/against), eDiscovery platform.
...
- Facts from `the installed practice profile for litigation-legal` → key facts for us, that this witness can establish
