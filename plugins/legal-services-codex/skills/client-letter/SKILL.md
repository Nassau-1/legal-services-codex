...
> Source: `plugins/vertical-plugins/legal-clinic/skills/client-letter/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /client-letter

1. Load `the installed practice profile for legal-clinic` → plain-language standards, supervision style, clinic contact info.
...
`the installed practice profile for legal-clinic` → plain-language standards, supervision style, clinic contact info.
...
Sending a letter to a client is a consequential action. This plugin's gate is the supervision workflow described in `## Supervision style` in `the installed practice profile for legal-clinic`, reinforced by the Part 0 role check that confirms a licensed supervising attorney owns the clinic setup. That gate still holds: every letter clears review before it leaves the clinic.

Before sending any of the letters above, confirm:

1. The draft has been reviewed per the supervision protocol in `the installed practice profile for legal-clinic` (queue / flag / lighter-touch).
...
Per `the installed practice profile for legal-clinic` standards. Short sentences. No jargon. Reading level target enforced. If a template above includes a legal term the client might not know, explain it the first time: "We filed your 'answer' — that's the document that tells the court your side of the story."

## Supervision routing

Per `the installed practice profile for legal-clinic`. Routine correspondence may or may not be a flag trigger depending on the supervision style the professor chose. If lighter-touch: these go out after student review without a queue step. If formal queue: even routine letters queue.
