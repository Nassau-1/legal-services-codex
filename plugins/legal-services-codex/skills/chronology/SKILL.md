...
> Source: `plugins/vertical-plugins/litigation-legal/skills/chronology/SKILL.md`.
...
2. Load `the installed practice profile for litigation-legal` → Document storage sources, default matter folder pattern.
3. Follow the workflow and reference below.
4. Identify sources in order: user-provided paths this session, default matter folder, declared sources from `the installed practice profile for litigation-legal`.
