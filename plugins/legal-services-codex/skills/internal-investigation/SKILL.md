...
> Source: `plugins/vertical-plugins/employment-legal/skills/internal-investigation/SKILL.md`.
...
Prepend the work-product header from `the installed practice profile for employment-legal` → `## Outputs` (it differs by user role — see `## Who's using this`). Every file, log, memo, and summary produced by this skill opens with that header.
...
Read `the installed practice profile for employment-legal` → escalation table, any investigation protocols noted.
