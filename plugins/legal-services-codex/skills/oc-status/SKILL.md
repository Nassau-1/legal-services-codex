...
> Source: `plugins/vertical-plugins/litigation-legal/skills/oc-status/SKILL.md`.
...
2. Load `the installed practice profile for litigation-legal` → outside counsel directive style, signer defaults, budget posture.
...
- `the installed practice profile for litigation-legal` → outside counsel directive style, signer name/email, budget posture
...
**Subject:** per house convention (from `the installed practice profile for litigation-legal` outside counsel directive style; fallback: `[Matter: [matter name]] — Weekly status update`)
...
4. **Budget** — [monthly / quarterly / on-request per `the installed practice profile for litigation-legal` budget posture]. Where are we against [budget authorization from matter.md]? Any variance to flag?
...
[Signoff — name, role, contact. From `the installed practice profile for litigation-legal` signer default for OC directives.]
```

Adapt tone per `the installed practice profile for litigation-legal` outside counsel directive style — some shops are "dear counsel" formal; others are first-name-and-bullets. Match.
...
**From:** [signer name / email from `the installed practice profile for litigation-legal`]
**Subject:** [subject line]

> The work-product header above applies to this internal record. The outgoing email body below goes to outside counsel on a retained matter, which is itself a privileged communication — apply the house privilege marking (`the installed practice profile for litigation-legal` privilege conventions) at the top of the email sent, typically `Privileged & Confidential — Attorney-Client Communication / Attorney Work Product`, not this internal work-product header.
...
- **Enforce a minimum template.** If the house tone is "one line, first name, done," the draft honors that and skips the bulleted structure. Match `the installed practice profile for litigation-legal`.
