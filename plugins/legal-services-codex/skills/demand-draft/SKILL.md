...
> Source: `plugins/vertical-plugins/litigation-legal/skills/demand-draft/SKILL.md`.
...
2. Load `the installed practice profile for litigation-legal` → demand-letter practice, house style, seed-doc table.
...
5. Template select: seed doc if provided in `the installed practice profile for litigation-legal`; else soft template for the demand type.
...
Check `the installed practice profile for litigation-legal` → Demand-letter practice → seed-doc table for the intake's demand type.
...
7. **Privilege markings per house style.** Apply `the installed practice profile for litigation-legal` privilege conventions exactly.
...
**Before the letter is sent (the consequential act):** Read `## Who's using this` in `the installed practice profile for litigation-legal`. If the Role is Non-lawyer:
...
- **Default yes if ANY of:**
  ...
  - Desired outcome $$ ≥ `the installed practice profile for litigation-legal` medium-severity band
