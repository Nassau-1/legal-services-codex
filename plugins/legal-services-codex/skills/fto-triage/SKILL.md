...
> Source: `plugins/vertical-plugins/ip-legal/skills/fto-triage/SKILL.md`.
...
Before running triage, read `the installed practice profile for ip-legal`. Pull:
...
Prepend the work-product header from `the installed practice profile for ip-legal` `## Outputs`. Mark the document as privileged if the role is lawyer; see the non-lawyer gate below if not.
