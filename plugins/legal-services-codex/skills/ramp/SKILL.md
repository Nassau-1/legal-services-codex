...
> Source: `plugins/vertical-plugins/legal-clinic/skills/ramp/SKILL.md`.
...
1. Check `the installed practice profile for legal-clinic` is set up. If placeholders: "Ask [professor] to run `/legal-clinic:cold-start-interview` first."
...
`the installed practice profile for legal-clinic` → clinic profile, practice areas, jurisdiction, handbook path, supervision style, practice-area templates.
...
Read from `the installed practice profile for legal-clinic` and the ingested handbook. Cover, interactively:
