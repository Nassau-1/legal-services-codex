...
> Source: `plugins/vertical-plugins/ai-governance-legal/skills/policy-monitor/SKILL.md`.
...
1. Read `the installed practice profile for ai-governance-legal` → outputs folder path, AI policy document, last sweep date.
...
6. Flag any use cases in outputs not yet added to the `the installed practice profile for ai-governance-legal` registry.
7. Present results to the human. Only after acknowledgment, update `Last policy sweep` and `gaps_found` in `the installed practice profile for ai-governance-legal`.

**Direct query mode** (with description argument):
1. Read `the installed practice profile for ai-governance-legal` → current policy commitments, use case registry, actual policy document.
...
→ **AI policy document**. The commitments in `the installed practice profile for ai-governance-legal` are a summary; the actual
...
**Do not update `Last policy sweep` or `gaps_found` automatically.** After the sweep results are presented, wait for the human to acknowledge them ("sweep acknowledged," "results reviewed," or equivalent). Only then update `the installed practice profile for ai-governance-legal`:
...
- Extract: vendor added, data use terms agreed to, any AI-specific provisions
  accepted that differ from standard positions in `the installed practice profile for ai-governance-legal`
...
[Any use cases approved since the last sweep that aren't yet in the `the installed practice profile for ai-governance-legal`
...
[If this use case isn't in the registry: "Add to `the installed practice profile for ai-governance-legal` → Use case registry:"]
```
