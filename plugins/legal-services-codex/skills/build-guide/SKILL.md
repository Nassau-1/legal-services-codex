...
> Source: `plugins/vertical-plugins/legal-clinic/skills/build-guide/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /build-guide

1. Load `the installed practice profile for legal-clinic` → role (must be Supervising attorney), practice areas, jurisdiction.
...
This is a supervisor skill. Read `the installed practice profile for legal-clinic` → `## Who's using this` → Role. If the role is not "Supervising attorney," say:
