...
> Source: `plugins/vertical-plugins/product-legal/skills/marketing-claims-review/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /marketing-claims-review

1. Load `the installed practice profile for product-legal` → Marketing claims standards.
...
Read `the installed practice profile for product-legal` → `## Marketing claims`:
...
Per `the installed practice profile for product-legal` — if comparative claims are "never," flag all of them. If "allowed with substantiation," check for the substantiation.
...
Prepend the work-product header from `the installed practice profile for product-legal` `## Outputs` (it differs by user role — see `## Who's using this`).
...
> **Before emitting "Ready to ship: Yes" (i.e., approving a claim for external use / publication):** Read `## Who's using this` in `the installed practice profile for product-legal`. If the Role is Non-lawyer:
>
