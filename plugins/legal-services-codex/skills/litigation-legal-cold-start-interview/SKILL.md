...
> Source: `plugins/vertical-plugins/litigation-legal/skills/cold-start-interview/SKILL.md`.
...
1. Check `the installed practice profile for litigation-legal`. If already populated and no `--redo`, ask before overwriting.
...
6. Write `the installed practice profile for litigation-legal`. Date the footer.
7. Confirm with the user before finalizing: "Here's what I captured — anything wrong?"

## Flags

- `--redo` — re-run the full interview and overwrite `the installed practice profile for litigation-legal`.
- `--check-integrations` — re-scan available MCP connectors and refresh the `## Available integrations` table in `the installed practice profile for litigation-legal` without re-running the full interview. Use after setting up a new connector (DMS, document storage, Gmail, scheduled-tasks, CLM).
...
Read `the installed practice profile for litigation-legal`:
...
> "Your practice profile is at `the installed practice profile for litigation-legal` — a plain text file you can read and edit directly. Anything you answered can be changed:
>
