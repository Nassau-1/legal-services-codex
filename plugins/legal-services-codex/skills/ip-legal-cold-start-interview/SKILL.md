...
> Source: `plugins/vertical-plugins/ip-legal/skills/cold-start-interview/SKILL.md`.
...
Runs the cold-start interview. First run writes `the installed practice profile for ip-legal`; subsequent runs with `--redo` re-interview and show a diff before overwriting.

## Instructions

1. **Check current state:** Read `the installed practice profile for ip-legal`. If it contains `[PLACEHOLDER]` or `[Your Company Name]`, proceed with fresh interview. If populated and `--redo` not passed, ask: "Looks like you're already set up. Want to re-run the interview? This will overwrite `the installed practice profile for ip-legal` (I'll show you a diff first)."
...
6. **Write `the installed practice profile for ip-legal`** (create parent directories as needed) per the structure below. Use the lawyer's own words where possible.
...
8. **Show summary + propose next steps:**
   - "Here's what I heard — `the installed practice profile for ip-legal` is written. What did I get wrong?"
...
Re-runs the integration availability check (IP management system, patent research, legal research, document storage, Slack) and updates `## Available integrations` in `the installed practice profile for ip-legal`. Does not re-interview. Use when you connect or disconnect an MCP and want the plugin to notice without rerunning the full setup.
...
Read `the installed practice profile for ip-legal`:
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/ip-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `the installed practice profile for ip-legal` with a `<!-- SETUP PAUSED AT: [section name] — run /ip-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
Write to `the installed practice profile for ip-legal` (create parent directories as needed). If the user shared a portfolio export, also seed `~/.claude/plugins/config/claude-for-legal/ip-legal/portfolio.yaml` with the extracted registrations.
...
4. **Close with a note on changeability.** End with something like:
   ...
   > "Done. Your practice profile is at `the installed practice profile for ip-legal` — it's a plain text file you can read and edit directly. Anything you answered can be changed:
   >
