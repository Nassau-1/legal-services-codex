...
> Source: `plugins/vertical-plugins/legal-clinic/skills/draft/SKILL.md`.
...
1. Load `the installed practice profile for legal-clinic` → practice-area templates, jurisdiction, local rules, supervision style.
...
`the installed practice profile for legal-clinic` → practice areas, practice-area templates, jurisdiction (state + local court + any local rules ingested), supervision style.
...
Match the request to the clinic's template set (from `the installed practice profile for legal-clinic`). Common set by practice area:
...
Per `the installed practice profile for legal-clinic` jurisdiction:
...
Filing a document with a court or agency is a consequential action. The gate is the supervision workflow in `## Supervision style` in `the installed practice profile for legal-clinic`, reinforced by the Part 0 role check that confirms a licensed supervising attorney owns the clinic setup. Court filings always route through supervision before filing, regardless of the supervision-style choice.

Per `the installed practice profile for legal-clinic` supervision style:
