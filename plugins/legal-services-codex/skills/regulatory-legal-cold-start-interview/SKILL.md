...
> Source: `plugins/vertical-plugins/regulatory-legal/skills/cold-start-interview/SKILL.md`.
...
4. Write `the installed practice profile for regulatory-legal` (creating parent directories as needed) with watchlist + materiality threshold.
...
Read `the installed practice profile for regulatory-legal`:
...
- **Pause and resume.** Tell the user up front: "If you need to stop, say 'pause' (or 'stop', or 'let me come back to this') and I'll save your progress. Run `/regulatory-legal:cold-start-interview` again later and I'll pick up where you left off." When the user pauses, write a partial configuration to `the installed practice profile for regulatory-legal` with a `<!-- SETUP PAUSED AT: [section name] — run /regulatory-legal:cold-start-interview to resume -->` comment at the top and `[PENDING]` markers (distinct from `[PLACEHOLDER]`) on unanswered fields. When setup re-runs and finds a paused config, greet the user: "Welcome back. You paused at [section]. Your earlier answers are saved. Pick up where we left off, or start over?" Do not re-ask questions already answered.
...
- Close with the changeability note:
  ...
  > "Done. Your configuration is at `the installed practice profile for regulatory-legal` — a plain-text file you can read and edit directly. Anything you answered can be changed:
  >
