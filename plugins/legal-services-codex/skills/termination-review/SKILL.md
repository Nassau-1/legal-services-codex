...
> Source: `plugins/vertical-plugins/employment-legal/skills/termination-review/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /termination-review

1. Load `the installed practice profile for employment-legal` → termination review triggers, high-risk flags, severance practice, jurisdiction rules.
...
`the installed practice profile for employment-legal` → termination review triggers, high-risk flags, standard severance,
jurisdiction table.

## Output header

Prepend the work-product header from `the installed practice profile for employment-legal` → `## Outputs` (it differs by user role — see `## Who's using this`). Match the memo format from seed term memos referenced in that config where one exists. The work-product header is always first.
...
This is the most important step. Check every flag from `the installed practice profile for employment-legal`. Default
set:
...
1. The employee works in a state with a high exempt salary threshold — **CA,
   ...
   `the installed practice profile for employment-legal` →
...
**Any flag fires → escalate per `the installed practice profile for employment-legal` before the term proceeds.** Not
after. Before.
...
Per `the installed practice profile for employment-legal` → standard severance:
...
**Before producing a "Go" recommendation or a term-day checklist marked ready:** Read `## Who's using this` in `the installed practice profile for employment-legal`. If the Role is **Non-lawyer**:
