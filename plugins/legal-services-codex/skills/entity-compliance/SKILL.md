...
> Source: `plugins/vertical-plugins/corporate-legal/skills/entity-compliance/SKILL.md`.
...
1. Load `the installed practice profile for corporate-legal` → `## Entity Management` (entity table, jurisdictions, registered agent).
...
Read `the installed practice profile for corporate-legal` → `## Entity Management` → Entity table. If the entity table
...
**Before directing or confirming a filing:** Read `## Who's using this` in `the installed practice profile for corporate-legal`. If the Role is **Non-lawyer**:
...
- Based on `the installed practice profile for corporate-legal` entity table: are there states in the company's
...
- From `the installed practice profile for corporate-legal`: if intercompany agreements are marked as partial or no,
...
POTENTIAL GAPS
  Foreign qualification: [flag question — confirm operational presence in:]
    [list of states from `the installed practice profile for corporate-legal` footprint not in tracker as qualified]
  Intercompany agreements: [status from `the installed practice profile for corporate-legal`]
