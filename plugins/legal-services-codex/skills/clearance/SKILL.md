...
> Source: `plugins/vertical-plugins/ip-legal/skills/clearance/SKILL.md`.
...
1. Read `the installed practice profile for ip-legal`. If it
...
Before running clearance, read `the installed practice profile for ip-legal`. Pull:
...
If `the installed practice profile for ip-legal` contains `[PLACEHOLDER]` or `[Your Company Name]`, surface this bounce:
...
Read `## Available integrations` from `the installed practice profile for ip-legal`:
...
Per the decision posture in `the installed practice profile for ip-legal`:
...
Prepend the work-product header from `the installed practice profile for ip-legal` `## Outputs`.
...
- [routing per `the installed practice profile for ip-legal` —
