...
> Source: `plugins/vertical-plugins/commercial-legal/skills/stakeholder-summary/SKILL.md`.
...
Read `the installed practice profile for commercial-legal` → `## House style` → who reads stakeholder summaries, how long should they be. If not specified, default to: procurement or a department head, two paragraphs max, no legal terms of art.
...
**To the stakeholder:** Via whatever channel `the installed practice profile for commercial-legal` says. If Slack, keep it under 150 words. If email, the format above is fine as-is.
