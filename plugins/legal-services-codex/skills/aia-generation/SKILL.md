---
name: aia-generation
description: "Run an AI impact assessment \u2014 structured intake, risk analysis, regulatory classification per regime in scope, policy consistency diff, and recommendation with conditions. Uses the house-style structure learned from the seed impact assessment in `the installed practice profile for ai-governance-legal`. Use when user says \"impact assessment for\", \"assess this AI use case\", \"run an AIA\", \"generate an AIA\", \"we need to document this AI system\", \"AI risk assessment for X\", or follows a conditional triage result."
---

> Generated by `scripts/build_codex_port.py` on `2026-05-14T16:29:15.363024+02:00`.
> Source: `plugins/vertical-plugins/ai-governance-legal/skills/aia-generation/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /aia-generation

1. Read `the installed practice profile for ai-governance-legal`. Confirm impact assessment house style is populated.
...
6. Policy diff against `the installed practice profile for ai-governance-legal` AI policy commitments.
...
- Is the use case in the elevated or high governance tier per `the installed practice profile for ai-governance-legal`?
...
Before intake, determine which track to run. The tier definitions and the fast-track criteria come from `the installed practice profile for ai-governance-legal` (`## Use case registry` and `## Governance tiers`), not from any hardcoded regime-specific framework.
...
**Fast track vs. full assessment:** `the installed practice profile for ai-governance-legal` defines what qualifies for abbreviated treatment. If `the installed practice profile for ai-governance-legal` doesn't define fast-track criteria, default to full assessment and ask the user what criteria they want captured for next time.
...
For each regime in `the installed practice profile for ai-governance-legal` → `## Regulatory footprint` that applies to this system — **plus any regime surfaced by the re-derivation above** — research the currently operative risk classification framework and determine where the system lands.
...
**Use the seed structure from `the installed practice profile for ai-governance-legal`.** If none was captured, use this default:
...
|---|---|---|
| [commitment from `the installed practice profile for ai-governance-legal` AI policy section] | 🟢 / 🟡 / 🟠 / 🔴 | |
...
**Before certifying the AIA (the Sign-off step, marking Status: APPROVED):** Read `## Who's using this` in `the installed practice profile for ai-governance-legal`. If the Role is Non-lawyer:
...
Every assessment should cross-check against the AI policy commitments in `the installed practice profile for ai-governance-legal`.
Common drift:
