...
> Source: `plugins/vertical-plugins/ip-legal/skills/ip-clause-review/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /ip-clause-review

Reviews the IP clauses in an agreement against the practice profile in `the installed practice profile for ip-legal`. Flags assignment gaps, ownership ambiguity, license-scope issues, and IP warranty/indemnity problems. Produces a memo with per-clause findings, prioritized by risk, with suggested redline language where appropriate.

## Instructions

1. **Load `the installed practice profile for ip-legal`.** If placeholders present, stop and prompt: "Run `/ip-legal:cold-start-interview` first — I need to learn your practice profile before I can review IP clauses against it."
...
**Before reading the agreement, read `the installed practice profile for ip-legal`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
**Escalation:** Per `the installed practice profile for ip-legal`, assignment-scope gaps escalate to [approver].
```
...
Prepend the work-product header from `the installed practice profile for ip-legal` → `## Outputs` (it differs by user role — see `## Who's using this`).
