...
> Source: `plugins/vertical-plugins/privacy-legal/skills/dsar-response/SKILL.md`.
...
1. Load `the installed practice profile for privacy-legal` → DSAR process (systems list, verification method, SLA).
...
Read `the installed practice profile for privacy-legal` → `## DSAR process`. That section has:
...
Per the method in `the installed practice profile for privacy-legal`. Common approaches:
...
Walk the systems list from `the installed practice profile for privacy-legal`. For each system:
...
**Before proceeding to send either letter to the data subject:** Read `## Who's using this` in `the installed practice profile for privacy-legal`. If the Role is Non-lawyer:
...
> **Note:** Both DSAR letters are externally-facing deliverables sent to the data subject. Do **not** include the work-product header from `the installed practice profile for privacy-legal` `## Outputs` on either letter. Internal notes, logs, and exemption analyses that accompany the letters are attorney work product — keep those separate and prepend the work-product header per `the installed practice profile for privacy-legal` `## Outputs` (which differs by user role — see `## Who's using this`).
...
Per `the installed practice profile for privacy-legal` → Escalation table, escalate when:
...
If `the installed practice profile for privacy-legal` → `## DSAR process` records an internal SLA that is tighter than the legal deadline, use the internal SLA and note the legal backstop.
