...
description: review of an inbound vendor agreement against the team playbook in `the installed practice profile for commercial-legal`. Flags deviations, assesses risk, generates specific redline language, and routes to the right approver. Loaded by /commercial-legal:review when a vendor MSA, services agreement, or similar is detected.
---

> Generated by `scripts/build_codex_port.py` on `2026-05-14T16:29:15.363024+02:00`.
> Source: `plugins/vertical-plugins/commercial-legal/skills/vendor-agreement-review/SKILL.md`.
...
Read a vendor agreement against the playbook this team actually uses (in `the installed practice profile for commercial-legal`), find every term that deviates, and tell the lawyer what to do about each one — with specific redline language, not vague "consider revising."
...
**Before reading the contract, read `the installed practice profile for commercial-legal`.** If it's missing or still has placeholders, surface this bounce:
...
The playbook in `the installed practice profile for commercial-legal` is the source of truth. It tells you:
...
Check the "one thing" from `the installed practice profile for commercial-legal` first. If present:
...
For each playbook category in `the installed practice profile for commercial-legal`, find the corresponding contract section and compare.
...
**Playbook says:** [our standard position, quoted from `the installed practice profile for commercial-legal`]
...
**If they won't move:** [the fallback from `the installed practice profile for commercial-legal`, or "escalate to [person]"
...
| Level | Means |
|---|---|
| 🔴 Critical | Don't sign without fixing. A term on the team's "never accept" list in `the installed practice profile for commercial-legal`, or a deal-breaker. |
...
Severity is always applied *against `the installed practice profile for commercial-legal`*. If a term doesn't map cleanly to a playbook position, ask the user which bucket it belongs in and offer to record the answer in `the installed practice profile for commercial-legal`.
...
Check the escalation matrix in `the installed practice profile for commercial-legal` against:
...
**Before proceeding to send redlines to the counterparty:** Read `## Who's using this` in `the installed practice profile for commercial-legal`. If the Role is Non-lawyer:
...
**Before generating a signature envelope or routing for countersignature:** Read `## Who's using this` in `the installed practice profile for commercial-legal`. If the Role is Non-lawyer:
...
**Full memo (default):** As above. Goes in the [CLM] record or the Drive folder from `the installed practice profile for commercial-legal` house-style section.
...
- [ ] `the installed practice profile for commercial-legal` was loaded and quoted — not generic market positions
