...
**Before scanning dependencies, read `the installed practice profile for ip-legal`.** If it is missing or still contains placeholders, stop and run `/ip-legal:cold-start-interview`. The practice profile tells you:
...
Prepend the work-product header from `the installed practice profile for ip-legal` → `## Outputs` (differs by user role — see `## Who's using this`).
