...
> Source: `plugins/vertical-plugins/employment-legal/skills/wage-hour-qa/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /wage-hour-qa

1. Load `the installed practice profile for employment-legal` → jurisdictional footprint.
...
`the installed practice profile for employment-legal` → jurisdictional footprint. If the question doesn't specify a
