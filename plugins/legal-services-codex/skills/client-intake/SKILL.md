...
> Source: `plugins/vertical-plugins/legal-clinic/skills/client-intake/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /client-intake

1. Load `the installed practice profile for legal-clinic` → practice areas, intake templates, supervision style, flag triggers.
...
`the installed practice profile for legal-clinic` → practice areas, intake templates (per practice area if multiple), supervision style, jurisdiction, flag triggers.
...
Each practice area asks different questions. Use the template from `the installed practice profile for legal-clinic` for this area. Defaults if none provided:
...
Per whatever conflict-check process `the installed practice profile for legal-clinic` describes. At minimum:
...
Per `the installed practice profile for legal-clinic` supervision style and flag triggers. If formal queue or configurable flags are enabled, and a trigger is present (deadline mentioned, DV indicator, immigration status at issue, etc.), note the flag.
