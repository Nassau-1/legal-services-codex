...
> Source: `plugins/vertical-plugins/legal-clinic/skills/research-start/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /research-start

1. Load `the installed practice profile for legal-clinic` → jurisdiction, practice area.
...
`the installed practice profile for legal-clinic` → jurisdiction (state), practice areas.
...
1. Read `the installed practice profile for legal-clinic` → `## Seed documents`. Identify any item whose purpose or filename matches the research area (e.g., "Alameda UD filing guide" for a UD habitability question; a redacted sample case file in the same practice area; a prior memo on the same issue).
