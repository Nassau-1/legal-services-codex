...
> Source: `plugins/vertical-plugins/law-student/skills/bar-prep-questions/SKILL.md`.
...
1. Load `the installed practice profile for law-student` → bar jurisdiction, exam format (NextGen / traditional UBE / state-specific), weak subjects, prep course.
...
1. Load `the installed practice profile for law-student` and read the bar jurisdiction and bar date.
...
Subject distribution: weight toward weak subjects **within the subjects actually tested on the student's exam**. If `the installed practice profile for law-student` says weak on Evidence and Civ Pro, 60% of questions come from those.
