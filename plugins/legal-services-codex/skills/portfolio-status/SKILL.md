...
> Source: `plugins/vertical-plugins/litigation-legal/skills/portfolio-status/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /portfolio-status

1. Load `the installed practice profile for litigation-legal` → risk calibration (defines how to read the `risk:` field).
...
- `the installed practice profile for litigation-legal` — risk calibration (to interpret risk/materiality fields correctly)
