...
> Source: `plugins/vertical-plugins/ip-legal/skills/portfolio/SKILL.md`.
...
Output (prepend work-product header per `the installed practice profile for ip-legal` → Outputs):
...
`## Who's using this` in `the installed practice profile for ip-legal`. If the Role is **Non-lawyer**:
