...
> Source: `plugins/vertical-plugins/commercial-legal/skills/renewal-tracker/SKILL.md`.
...
**Before proceeding to accept or decline a renewal (including sending a non-renewal notice or letting an auto-renewal run past the cancel-by date):** Read `## Who's using this` in `the installed practice profile for commercial-legal`. If the Role is Non-lawyer:
...
The renewal-watcher agent in this plugin runs this skill on a schedule (weekly by default) and posts the "coming up" report to the channel named in `the installed practice profile for commercial-legal` → `## House style` → where work product goes. Mode 2 is the agent's primary output.
