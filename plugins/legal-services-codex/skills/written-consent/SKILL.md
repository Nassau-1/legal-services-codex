...
> Source: `plugins/vertical-plugins/corporate-legal/skills/written-consent/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /written-consent

1. Load `the installed practice profile for corporate-legal` → Board & Secretary (consents repository, resolution language, state of incorporation, board composition).
...
5. Search consents repository for closest precedent. If no repository: use seed consents from `the installed practice profile for corporate-legal`.
...
- `the installed practice profile for corporate-legal` → `## Board & Secretary`:
...
- **Signatories:** Full board, or a specific committee? If the `the installed practice profile for corporate-legal` written-consent scope says certain actions require a meeting rather than consent, flag it now.
...
Extract the format from the seed consents in `the installed practice profile for corporate-legal`. Note that no precedent search is available — the draft will follow house format but without substantive precedent matching. Flag this to the user:
...
Check the state of incorporation in `the installed practice profile for corporate-legal`. Research the written-consent requirements for that state before drafting:
...
If `the installed practice profile for corporate-legal` records a house position on any of these questions, apply it and note the legal backstop being relied on. Add a short "State-law notice" block to the output summarizing what you confirmed (or flagged) so the user isn't left wondering.

---

## Step 4.5: Consequential-action gate (execute consent)

**Before proceeding to output:** Read `## Who's using this` in `the installed practice profile for corporate-legal`. If the Role is **Non-lawyer**:
...
1. **The consent draft** — complete, ready to review and circulate. The executed written consent itself is a corporate record, not privileged; do not apply the work-product header to the consent as circulated. The drafting notes, signatory tracker, and analysis below are work product — prepend the work-product header from `the installed practice profile for corporate-legal` `## Outputs` (it differs by user role — see `## Who's using this`):
...
□ [Director Name 2]
□ [Director Name 3]
[etc. — pulled from board composition in `the installed practice profile for corporate-legal`]
