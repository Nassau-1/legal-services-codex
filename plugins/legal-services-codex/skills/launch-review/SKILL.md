...
> Source: `plugins/vertical-plugins/product-legal/skills/launch-review/SKILL.md`.
> This is a first-pass Codex export. Review `codex/compatibility-report.md` before relying on legacy runtime instructions.

# /launch-review

1. Load `the installed practice profile for product-legal` → framework + calibration. Stop if placeholders.
...
Read the PRD, check every category in this team's framework, calibrate against what actually blocks here (per `the installed practice profile for product-legal`), and output a review in house format. Goal: a PM reads it and knows exactly what has to happen before they ship.

## Load calibration

Read `the installed practice profile for product-legal`:
...
For each category in `the installed practice profile for product-legal` → Review framework. If the team doesn't have one, use the 8-category default below. The categories are stable framing concepts; within each category, research the regulatory regimes applicable to the product's sector, audience, and jurisdictions before calibrating severity. What blocks in one jurisdiction or sector may be routine in another — `the installed practice profile for product-legal` captures the team's calibration.
...
Format per `the installed practice profile for product-legal` → Launch review process → output format. Prepend the work-product header from `the installed practice profile for product-legal` `## Outputs` (it differs by user role — see `## Who's using this`). If no house format is specified:
...
> **Before emitting a "Clear to ship" or "Ship with conditions" call:** Read `## Who's using this` in `the installed practice profile for product-legal`. If the Role is Non-lawyer:
>
...
**Output 1 — Privileged launch review memo.** The full analysis assembled in Step 5: work-product header, bottom line, findings by category with risk rationale, action items, escalations, notes for next time, citation check. This is internal legal work product. Keep it in your matter file (Drive, DMS, or wherever `the installed practice profile for product-legal` says review docs go). Distribute only to people inside the privilege circle.
