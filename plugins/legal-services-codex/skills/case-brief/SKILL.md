...
> Source: `plugins/vertical-plugins/law-student/skills/case-brief/SKILL.md`.
...
1. Load `the installed practice profile for law-student` → outline/brief preferences.
...
`the installed practice profile for law-student` → outline/brief preferences (format, depth), learning style.
...
Per the student's format in `the installed practice profile for law-student`. If none captured, default:
...
Per `the installed practice profile for law-student` — some students want one-line briefs (rule + cite), some want full treatment. Match their format.
